...
export class E2ETestRunner {
  ...
  private async testImportDocumentWorkflow(): Promise<E2ETestResult> {
    ...
    try {
      ...
      const step2: E2ETestStep = {
        stepName: '打开文档',
        passed: doc !== null,
        message: doc !== null ? '文档打开成功' : '文档打开失败',
  ...
  private async testGenerateChapterWorkflow(): Promise<E2ETestResult> {
    ...
    try {
      ...
      const generateCommandExists = commands.includes('novelAssistant.generateChapter');

      const step1: E2ETestStep = {
        stepName: '检查生成章节命令',
        passed: generateCommandExists,
        message: generateCommandExists ? '生成章节命令已注册' : '生成章节命令未注册',
        duration: Date.now() - step1Start
      };
      steps.push(step1);
      this.logger.log(`  ${step1.passed ? '✅' : '❌'} ${step1.stepName}: ${step1.message}`);
  ...
  private async testAnalyzeChapterWorkflow(): Promise<E2ETestResult> {
    ...
    try {
      const step1Start = Date.now();
      const commands = await vscode.commands.getCommands();
      const analyzeCommandExists = commands.includes('novelAssistant.analyzeChapter');

      const step1: E2ETestStep = {
        stepName: '检查分析章节命令',
        passed: analyzeCommandExists,
        message: analyzeCommandExists ? '分析章节命令已注册' : '分析章节命令未注册',
        duration: Date.now() - step1Start
      };
      steps.push(step1);
      this.logger.log(`  ${step1.passed ? '✅' : '❌'} ${step1.stepName}: ${step1.message}`);
      ...
      if (apiKey !== '' && apiBase !== '' && model !== '') {
        try {
          ...
          const testChapter = '这是测试章节内容，包含一些角色对话和情节发展。';
  ...
  private async testViewDataWorkflow(): Promise<E2ETestResult> {
    ...
    try {
      const step1Start = Date.now();
      await vscode.commands.executeCommand('novelAssistant.showForeshadows');
      ...
      const step2Start = Date.now();
      await vscode.commands.executeCommand('novelAssistant.showSummaries');
      ...
      await vscode.commands.executeCommand('novelAssistant.showOutlines');
  ...
  private async testImportFolderWorkflow(): Promise<E2ETestResult> {
    ...
    try {
      const step1Start = Date.now();
      const commands = await vscode.commands.getCommands();
      const importFolderCommandExists = commands.includes('novelAssistant.importFolder');

      const step1: E2ETestStep = {
        stepName: '检查导入文件夹命令',
        passed: importFolderCommandExists,
        message: importFolderCommandExists ? '导入文件夹命令已注册' : '导入文件夹命令未注册',
        duration: Date.now() - step1Start
      };
      steps.push(step1);
      this.logger.log(`  ${step1.passed ? '✅' : '❌'} ${step1.stepName}: ${step1.message}`);
